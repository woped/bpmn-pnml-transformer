from transformer.bpmn.bpmn import Process
...
def extend_subprocess(subprocesses: set[Process], parent_process: Process):
    for subprocess in subprocesses.copy():
        ...
        for a in process_incoming:
            parent_process.add_flow(parent_process.get_node(a.sourceRef), start_event, id=a.id, name=a.name)
        for a in process_outgoing:
            parent_process.add_flow(end_event, parent_process.get_node(a.targetRef), id=a.id, name=a.name)
