...
from transformer.bpmn.bpmn import AndGateway, Flow, OrGateway, Process, Task, XorGateway
...
def traverse_matching_gw(
    ...
    for flow_id in outgoing_arcs:
        r = traverse_matching_gw(bpmn_helper, stack, split_ids, join_ids, visited_arcs, flow_id)
        if r is not None:
            return r
    return None
...
def find_matching_gateways(bpmn_helper: Process, inclusive_gateways: list[OrGateway]):
    ...
    for split in splits:
        ...
        for out_flow_id in outgoing_flows:
            r = traverse_matching_gw(bpmn_helper, [split], split_ids, join_ids, set(), out_flow_id)
            if r is None:
                raise Exception("Could not find matching splits and joins for OR-Gateways")
            matches.append(
                InclusiveGatewayBridge(split, r[1], bpmn_helper.get_flow(out_flow_id), bpmn_helper.get_flow(r[0]))
            )
    return matches


def clone_flow(
    flow: Flow, new_source: GenericBPMNNode | None = None, new_target: GenericBPMNNode | None = None, new_id=None
):
...
def add_xors_and_activities(bpmn: Process, and_gw: ParallelGatewayBridge):
    ...
    split_to_xor_arc = Flow(sourceRef=and_gw.split.id, targetRef=xor_split.id, id=and_gw.split.id + xor_split.id)
    xor_to_join_arc = Flow(sourceRef=xor_join.id, targetRef=and_gw.join.id, id=xor_join.id + and_gw.join.id)
    ...
    xor_split_to_silent_activity_arc = Flow(
        sourceRef=xor_split.id, targetRef=silent_activity.id, id=xor_split.id + silent_activity.id
    )
    silent_activity_to_xor_join_arc = Flow(
        sourceRef=silent_activity.id, targetRef=xor_join.id, id=silent_activity.id + xor_join.id
    )
...
def replace_inclusive_to_parallel(bpmn: Process, gw: OrGateway):
    ...
    for arc in in_arcs:
        ...
        new_arc = bpmn.add_flow(bpmn.get_node(arc.sourceRef), pw_gw, "OR_" + arc.id, arc.name)
        flow_map[arc.id] = new_arc
    for arc in out_arcs:
        bpmn.remove_flow(arc)
        new_arc = bpmn.add_flow(pw_gw, bpmn.get_node(arc.targetRef), "OR_" + arc.id, arc.name)
        flow_map[arc.id] = new_arc
    bpmn.remove_node(gw)
...
def replace_inclusive_gateways(in_bpmn: Process):
    ...
    inclusive_gateways = [node for node in nodes if type(node) == OrGateway]
