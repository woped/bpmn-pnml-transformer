...
from transformer.bpmn.base import Gateway, GenericBPMNNode
from transformer.bpmn.bpmn import Flow, Process
...
def get_gateways(bpmn: Process):
    ...
    gateways = cast(set[Gateway], {node for node in nodes if issubclass(type(node), Gateway)})
    return gateways
