...
from transformer.pnml.pnml import Place, Pnml, Transition
...
def transform_bpmn_to_petrinet(bpmn: Process, is_workflow_net: bool = False):
    ...
    if is_workflow_net:
        workflow_nodes: set[GenericBPMNNode] = set([x for x in nodes if type(x) in to_handle_as_workflow_element])
        nodes = nodes.difference(workflow_nodes)

    # handle normals nodes
    for node in nodes:
        if type(node) == Task or type(node) == AndGateway:
            net.add_element(
                Transition.create(
                    id=node.id,
                    name=(
                        node.name if node.name != "" or node.get_in_degree() > 1 or node.get_out_degree() > 1 else None
                    ),
                )
            )
        elif (
            type(node) == OrGateway
            or type(node) == XorGateway
            or type(node) == StartEvent
            or type(node) == EndEvent
            or type(node) == GenericBPMNNode
        ):
    ...
    # handle reaining flows
    for flow in bpmn.flows:
        source = net.get_node_or_none(flow.sourceRef)
        target = net.get_node_or_none(flow.targetRef)
        if source is None or target is None:
            continue
        if type(source) == Place and type(target) == Place:
            t = net.add_element(Transition(id=create_silent_node_name(source.id, target.id)))
            net.add_arc(source, t)
            net.add_arc(t, target)
        elif type(source) == Transition and type(target) == Transition:
...
def bpmn_to_workflow_net(bpmn: BPMN):
    apply_preprocessing(
        bpmn.process, [replace_inclusive_gateways, preprocess_gateways, insert_temp_between_adjacent_subprocesses]
    )
