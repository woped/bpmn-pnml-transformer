...
from transformer.pnml.base import (
    GenericNetNode,
    Graphics,
    Inscription,
    Name,
...
class Net(BaseModel, tag="net"):
    ...
    # Hidden helper structure
    _temp_elements: dict[str, NetElement] = PrivateAttr(default_factory=dict)
    _type_map: dict[Any, set[GenericNetNode]] = PrivateAttr(default_factory=dict)
    ...
    def _init_reference_structures(self):
        self._type_map = {Place: self.places, Transition: self.transitions, Page: self.pages}  # type:ignore
    ...
    def add_arc_with_handle_same_type(self, source: NetElement, target: NetElement):
        if type(source) == Place and type(target) == Place:
            t = self.add_element(Transition(id=create_silent_node_name(source.id, target.id)))
...
class Pnml(BaseModel, tag="pnml"):
    ...
    def from_xml_str(xml_content: str):
        net = Pnml.from_xml(bytes(xml_content, encoding="utf-8"))  # type: ignore
        return net
