...
from transformer.bpmn.base import Gateway
...
from transformer.pnml.pnml import Net, Place, Pnml, Transition
...
def remove_unnecessary_gateways(bpmn: Process):
    gw_nodes = {node for node in bpmn._flatten_node_typ_map() if issubclass(type(node), Gateway)}
...
def transform_petrinet_to_bpmn(net: Net):
    ...
    for transition in transitions:
        in_degree, out_degree = net.get_in_degree(transition), net.get_out_degree(transition)
    ...
    handle_workflow_subprocesses(net, bpmn, to_handle_subprocesses, transform_petrinet_to_bpmn)
