...
from transformer.bpmn.bpmn import BPMN, AndGateway, Process, XorGateway
from transformer.pnml.base import NetElement
from transformer.pnml.pnml import Arc, Net, Page, Transition
...
def handle_workflow_subprocesses(
    net: Net, bpmn: Process, to_handle_subprocesses: list[Transition], caller_func: Callable[[Net], BPMN]
):
    for subprocess_transition in to_handle_subprocesses:
        ...
        inner_source_id, inner_sink_id = page_net.get_element(outer_source_id), page_net.get_element(outer_sink_id)

        if page_net.get_in_degree(inner_source_id) > 0 or page_net.get_out_degree(inner_sink_id) > 0:
            raise Exception(
...
def handle_workflow_operators(net: Net, bpmn: Process, to_handle_operators: list[WorkflowOperatorWrapper]):
    for workflow_operator in to_handle_operators:
        ...
        if workflow_operator.t in {
            ...
            new_gateway = bpmn.add_node(AndGateway(id=workflow_operator.id, name=workflow_operator.name))
            ...
            new_gateway = bpmn.add_node(XorGateway(id=workflow_operator.id, name=workflow_operator.name))
