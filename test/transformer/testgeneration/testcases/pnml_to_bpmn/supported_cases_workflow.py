...
from transformer.bpmn.bpmn import (
    BPMN,
    AndGateway,
    EndEvent,
    StartEvent,
...
from transformer.pnml.pnml import Page, Place, Pnml, Transition
...
def gateway_parallel_join_split():
    ...
    and_split = create_operator_transition(gw_split, 1, WorkflowBranchingType.AndSplit, gw_split)
    and_join_split = create_operator_transition(gw_both, 1, WorkflowBranchingType.AndJoinSplit, gw_both)
    and_join = create_operator_transition(gw_join, 1, WorkflowBranchingType.AndJoin, gw_join)
...
def gateway_exclusive_join_split():
    ...
    xor_split_1 = create_operator_transition(gw_split, 1, WorkflowBranchingType.XorSplit, gw_split)
    xor_split_2 = create_operator_transition(gw_split, 2, WorkflowBranchingType.XorSplit, gw_split)

    xor_join_split_place = create_operator_place(gw_both, WorkflowBranchingType.XorJoinSplit)
    xor_join_split_in_1 = create_operator_transition(gw_both, 1, WorkflowBranchingType.XorJoinSplit, gw_both)
    xor_join_split_in_2 = create_operator_transition(gw_both, 2, WorkflowBranchingType.XorJoinSplit, gw_both)
    xor_join_split_out_1 = create_operator_transition(gw_both, 3, WorkflowBranchingType.XorJoinSplit, gw_both)
    xor_join_split_out_2 = create_operator_transition(gw_both, 4, WorkflowBranchingType.XorJoinSplit, gw_both)

    xor_join_1 = create_operator_transition(gw_join, 1, WorkflowBranchingType.XorJoin, gw_join)
    xor_join_2 = create_operator_transition(gw_join, 2, WorkflowBranchingType.XorJoin, gw_join)
    ...
    net = create_petri_net(
        ...
        [
            [
                ...
                Place.create(id=create_silent_node_name(xor_join_split_out_1.id, task_2)),
                Transition.create(id=task_2, name=task_2),
            ...
            [
                ...
                Transition.create(id=task_11, name=task_11),
                Place.create(id=create_silent_node_name(task_11, xor_join_split_in_2.id)),
                xor_join_split_in_2,
                xor_join_split_place,
                xor_join_split_out_2,
                Place.create(id=create_silent_node_name(xor_join_split_out_2.id, task_22)),
                Transition.create(id=task_22, name=task_22),
...
def gateway_side_by_side_xor_and():
    ...
    # Xor Part
    xor_split_1 = create_operator_transition(gw_split_start, 1, WorkflowBranchingType.XorSplit, gw_split_start)
    xor_split_2 = create_operator_transition(gw_split_start, 2, WorkflowBranchingType.XorSplit, gw_split_start)
    xor_join_1 = create_operator_transition(gw_join_start, 1, WorkflowBranchingType.XorJoin, gw_join_start)
    xor_join_2 = create_operator_transition(gw_join_start, 2, WorkflowBranchingType.XorJoin, gw_join_start)
    # And Part
    and_split = create_operator_transition(gw_split_end, 1, WorkflowBranchingType.AndSplit, gw_split_end)
    and_join = create_operator_transition(gw_join_end, 1, WorkflowBranchingType.AndJoin, gw_join_end)
...
def gateway_side_by_side_and_xor():
    ...
    # And Part
    and_split = create_operator_transition(gw_split_start, 1, WorkflowBranchingType.AndSplit, gw_split_start)
    and_join = create_operator_transition(gw_join_start, 1, WorkflowBranchingType.AndJoin, gw_join_start)
    # Xor Part
    xor_split_1 = create_operator_transition(gw_split_end, 1, WorkflowBranchingType.XorSplit, gw_split_end)
    xor_split_2 = create_operator_transition(gw_split_end, 2, WorkflowBranchingType.XorSplit, gw_split_end)
    xor_join_1 = create_operator_transition(gw_join_end, 1, WorkflowBranchingType.XorJoin, gw_join_end)
    xor_join_2 = create_operator_transition(gw_join_end, 2, WorkflowBranchingType.XorJoin, gw_join_end)
...
def xor_and_split():
    ...
    xor_join_and_split_in_1 = create_operator_transition(gw_both, 1, WorkflowBranchingType.XorJoinAndSplit, gw_both)
    xor_join_and_split_in_2 = create_operator_transition(gw_both, 2, WorkflowBranchingType.XorJoinAndSplit, gw_both)
    xor_join_and_split_out = create_operator_transition(gw_both, 3, WorkflowBranchingType.XorJoinAndSplit, gw_both)
    # And Part
    and_join = create_operator_transition(gw_join_end, 1, WorkflowBranchingType.AndJoin, gw_join_end)
    ...
    net = create_petri_net(
        ...
        [
            [
                ...
                Place.create(id=create_silent_node_name(xor_split_1.id, task_1)),
                Transition.create(id=task_1, name=task_1),
                Place.create(id=create_silent_node_name(task_1, xor_join_and_split_in_1.id)),
                xor_join_and_split_in_1,
                linking_place,
                xor_join_and_split_out,
                Place.create(id=create_silent_node_name(xor_join_and_split_out.id, task_2)),
                Transition.create(id=task_2, name=task_2),
                Place.create(id=create_silent_node_name(task_2, and_join.id)),
                and_join,
                end,
            ...
            [
                ...
                Place.create(id=create_silent_node_name(xor_split_2.id, task_11)),
                Transition.create(id=task_11, name=task_11),
                Place.create(id=create_silent_node_name(task_11, xor_join_and_split_in_2.id)),
                xor_join_and_split_in_2,
                linking_place,
            ],
            [
                xor_join_and_split_out,
                Place.create(id=create_silent_node_name(xor_join_and_split_out.id, task_22)),
                Transition.create(id=task_22, name=task_22),
                Place.create(id=create_silent_node_name(task_22, and_join.id)),
                and_join,
            ],
...
def and_xor_split():
    ...
    # Xor Part
    xor_join_1 = create_operator_transition(gw_xor_join_start, 1, WorkflowBranchingType.XorSplit, gw_xor_join_start)
    xor_join_2 = create_operator_transition(gw_xor_join_start, 2, WorkflowBranchingType.XorSplit, gw_xor_join_start)
    # Inner Part
    linking_place = create_operator_place(gw_both, WorkflowBranchingType.AndJoinXorSplit)
    and_join_xor_split_in = create_operator_transition(gw_both, 1, WorkflowBranchingType.AndJoinXorSplit, gw_both)
    and_join_xor_split_out_1 = create_operator_transition(gw_both, 2, WorkflowBranchingType.AndJoinXorSplit, gw_both)
    and_join_xor_split_out_2 = create_operator_transition(gw_both, 3, WorkflowBranchingType.AndJoinXorSplit, gw_both)
    # And Part
    and_split = create_operator_transition(gw_and_split_start, 1, WorkflowBranchingType.AndJoin, gw_and_split_start)
    ...
    net = create_petri_net(
        ...
        [
            [
                ...
                Place.create(id=create_silent_node_name(and_split.id, task_1)),
                Transition.create(id=task_1, name=task_1),
                Place.create(id=create_silent_node_name(task_1, and_join_xor_split_in.id)),
                and_join_xor_split_in,
                linking_place,
                and_join_xor_split_out_1,
                Place.create(id=create_silent_node_name(and_join_xor_split_out_1.id, task_2)),
                Transition.create(id=task_2, name=task_2),
                Place.create(id=create_silent_node_name(task_2, xor_join_1.id)),
                xor_join_1,
                end,
            ...
            [
                ...
                Place.create(id=create_silent_node_name(and_join_xor_split_out_2.id, task_22)),
                Transition.create(id=task_22, name=task_22),
...
def subprocess():
    ...
    net = create_petri_net(
        ...
        [
            [
                Place(id=se_id),
                Transition.create(subprocess_id, subprocess_name).mark_as_workflow_subprocess(),
                Place(id=ee_id),
            ],
        ],
    )
    ...
    net_page = Page(
        id=subprocess_id,
        net=create_petri_net("", [[Place(id=se_id), Transition.create(id=sb_t_id, name=sb_t_id), Place(id=ee_id)]]).net,
    )
    net_page.net.id = subprocess_id
    net.net.add_page(net_page)

    sub_bpmn = create_bpmn("temp", [[StartEvent(id=se_id), Task(id=sb_t_id, name=sb_t_id), EndEvent(id=ee_id)]])
    sub_bpmn.process.id = subprocess_id
    sub_bpmn.process.name = subprocess_name

    bpmn = create_bpmn(case, [[StartEvent(id=se_id), sub_bpmn.process, EndEvent(id=ee_id)]])
