...
def start_end():
    ...
    net = create_petri_net(
        case, [[Place(id=se_id), Transition(id=create_silent_node_name(se_id, ee_id)), Place(id=ee_id)]]
    )
...
def task():
    ...
    ee_id = "elem_2"
    task_id = "elem_3"
    bpmn = create_bpmn(case, [[StartEvent(id=se_id), Task(id=task_id, name=task_id), EndEvent(id=ee_id)]])
    net = create_petri_net(
        case,
        [
            [
...
def gateway_parallel():
    ...
    bpmn = create_bpmn(
        case,
        [
            [StartEvent(id=se_id), bpmn_gw_split, Task(id=task_id, name=task_id), bpmn_gw_join, EndEvent(id=ee_id)],
