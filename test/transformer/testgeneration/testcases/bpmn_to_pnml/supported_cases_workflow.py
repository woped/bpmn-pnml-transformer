...
def reduce_unnecessary_gw():
    ...
    net = create_petri_net(
        case,
        [[Place.create(id=se_id), Transition.create(id=create_silent_node_name(se_id, ee_id)), Place.create(id=ee_id)]],
    )
...
def subprocess():
    ...
    sub_bpmn = create_bpmn("temp", [[StartEvent(id=sb_se_id), Task(id=sb_t_id), EndEvent(id=sb_ee_id)]])
    ...
    net_page = Page(
        id=subprocess_id, net=create_petri_net("", [[Place(id=se_id), Transition(id=sb_t_id), Place(id=ee_id)]]).net
    )
