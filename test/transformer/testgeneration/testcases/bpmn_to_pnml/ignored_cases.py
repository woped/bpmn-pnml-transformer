...
from transformer.bpmn.bpmn import EndEvent, StartEvent, Task
...
def generate_helper_bpmn(case_name: str):
    ...
    ee_id = "elem_2"
    task_id = "elem_3"
    bpmn = create_bpmn(case_name, [[StartEvent(id=se_id), Task(id=task_id), EndEvent(id=ee_id)]])
...
def generate_temp_petri_net(case_name: str):
    ...
    ee_id = "elem_2"
    task_id = "elem_3"
    return create_petri_net(case_name, [[Place(id=se_id), Transition(id=task_id), Place(id=ee_id)]])
