...
def save_failed_bpmn_to_pnml_transformation(pn_expected: Pnml, pn_transformed: Pnml, bpmn: BPMN, case: str):
...
def save_failed_pnml_to_bpmn_transformation(bpmn_expected: BPMN, bpmn_transformed: BPMN, net: Pnml, case: str):
    def create_path(file_name: str = ""):
...
class TestPetriNetToBPMN(unittest.TestCase):
    def test_supported_elements(self):
        for bpmn_expected, net, case in supported_cases_pnml:
            ...
            if not equal:
                save_failed_pnml_to_bpmn_transformation(bpmn_expected, bpmn_transformed, net, case)
...
class TestBPMNToPetriNet(unittest.TestCase):
    ...
    def test_supported_elements(self):
        for bpmn, pn_expected, case in supported_cases_bpmn:
            ...
            if not equal:
                save_failed_bpmn_to_pnml_transformation(pn_expected, pn_transformed, bpmn, case)
            with self.subTest(case):
                self.assertTrue(equal, f"{case} should be equal\n{error}")
